...
class Font
{
	...
	{
		// log fatal error if fontpath does not exist
		auto fp = new FilePath(fontPath);

		if (!fp.exists)
			logger.fatal("Font " ~ fontPath ~ " does not exist!");
		...
		if ((nameStr in fontList) is null)
		{
			...
			version (UnpatentedHinting) {
				logger.info("Unpatented Font Hinting Version");
		...
		{
			logger.info("Using previously loaded font for " ~ nameStr);
	...
	{
		lineGap_ = rndlong(frac * getHeight);
	}
