...
struct Joysticks
{
	...
	*******************************************************************************/
	struct Joystick
	{
		...
		/// name
		char[] name() {	return this.isOpen ? fromStringz(SDL_JoystickName( SDL_JoystickIndex(this.joystick) )) : ""; }
	...
	void clear()
	{
		...
		{
			arc.internals.input.signals.signals.joyHotPlug(numJoysticks, nstick);
			numJoysticks = nstick;
		}		
	}
	...
	void handleButtonDown(inout SDL_Event event)
	{
		...
		// raise signal
		arc.internals.input.signals.signals.joyButtonDown(event.jbutton.which, event.jbutton.button);
	}
	...
	void handleButtonUp(inout SDL_Event event)
	{
		...
		// raise signal
		arc.internals.input.signals.signals.joyButtonUp(event.jbutton.which, event.jbutton.button);
	}
	...
	void handleAxisMove(inout SDL_Event event)
	{
		...
		arc.internals.input.signals.signals.joyAxisMove(event.jaxis.which, event.jaxis.axis, pos);
	}
} 
