...
{
	// setup logger
	logger = Log.getLogger("arc.sound");
}
...
{	protected:
	...
	this(char[] filename)
	{
		...
		{	
			...
			if (!filepath.exists())
			{
				logger.fatal("Sound File " ~ filename ~ " does not exist!");
			} 
			...
			if (!(filename in soundFileList))
			{
				...
				else if (data[0..4]=="OggS")
					sound_file = new VorbisFile(filename);
				else 
					logger.fatal("Unrecognized sound format '"~data[0..4]~"' for file '"~filename~"'.");
				...
				else logger.fatal("Sound must be 8 or 16 bit and mono or stero format.");
	...
	uint[] getBuffers()	{	return buffers;	}

	/// Get the number of buffers this sound was divided into
	uint getBuffersLength() {	return buffers.length;	}

	/// Get the number of buffers created for each second of this sound
	uint getBuffersPerSecond() {	return buffers_per_sec; }

	/// Get the length of the sound in seconds
	double getLength() {	return (8.0*sound_file.size)/(sound_file.bits*sound_file.frequency*sound_file.channels);	}

	/// Return the size of the uncompressed sound data, in bytes.
	uint getSize() {	return sound_file.size; }

	/// Get the filename this Sound was loaded from.
	char[] getSource() {	return sound_file.source;	}
		...
		{	// Allow inputs that are out of range to loop around
			...
			{	alDeleteBuffers(1, &buffers[i]);
				if (alIsBuffer(buffers[i]))
					logger.fatal("Sound buffer "~tango.text.convert.Integer.toString(i)~" of '"~sound_file.source~
...
private abstract class BaseSoundFile
{
	...
	abstract ubyte[] getBuffer(int offset, int size);
...
private class WaveFile : BaseSoundFile
{
	...
	{	
		...
		if (file[0..4] != "RIFF")
			logger.fatal("'"~filename~"' is not a RIFF file.");
		// Skip size value (4 bytes)
		if (file[8..12] != "WAVE")
			logger.fatal("'"~filename~"' is not a WAVE file.");
...
private class VorbisFile : BaseSoundFile
{
	...
	this(char[] filename)
	{	
		logger.info("Loading OGG Vorbis file " ~ filename); 
		super(filename);
...
private
{
	...
	{
		char[][] ignoreMethods =
			["aluF2L"[], "aluF2S", "aluCrossproduct",
				"aluDotproduct", "aluNormalize", "aluMatrixVector",
				"aluCalculateSourceParameters", "aluMixData",
				"aluSetReverb", "aluReverb", "vorbis_synthesis_idheader"];
