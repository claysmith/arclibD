...
module arc.x.blaze.dynamics.contact.generator.polyContact;
 ...
 * for Polygon-Polygon contacts.
 */
class PolyContact : Contact {

    this(Shape s1, Shape s2) {
        assert(s1.type == ShapeType.POLYGON);
        assert(s2.type == ShapeType.POLYGON);
        super(s1, s2);
        manifolds.length = 1;
    }

    static Contact create(Shape s1, Shape s2) {
        return new PolyContact(s1, s2);
    }
    ...
    void evaluate(ContactListener listener) {
        ...
        collidePolygons(manifolds[0], cast(Polygon) m_shape1, cast (Polygon) m_shape2);
